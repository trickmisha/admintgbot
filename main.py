...
async def drip_got_text(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        pass
    # Сохраняем entities для кастомных эмодзи
    entities_json = None
    if message.entities:
        try:
...
async def on_paid_channel_left(event: ChatMemberUpdated):
    ...
    if old_status in _MEMBER_LIKE and new_status not in _MEMBER_LIKE:
        ...
        for admin_id in ADMIN_IDS:
            asyncio.create_task(notify_and_delete(
                admin_id,
                f"🔴 VIP: {uname} отписался ({new_status})",
                delay=10
            ))
